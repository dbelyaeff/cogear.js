...
module.exports = {
	...
	async preload(){
		if(!cogear.webpackConfig){
			ora().fail("Webpack config must be loaded before pages preload starts.");
			process.exit()
		}
	...
	Please, make sure that at least one page file is available.`);
			...
			this.loader.succeed(`Pages are preloaded. (${prettyMs(now()-start)})`);	
	...
	async page(file,page){
		return new Promise(async(resolve,reject)=>{
			...
			if(file){
				...
				page.date = page.date || new Date(fs.statSync(page.filePath).birthtimeMs)
			} 
